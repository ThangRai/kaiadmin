...
if (isset($_POST['toggle_status'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE project SET is_active = ? WHERE id = ?");
        $stmt->execute([$is_active, $project_id]);
...
if (isset($_POST['update_position'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE project SET position = ? WHERE id = ?");
        $stmt->execute([$new_position, $project_id]);
...
if (isset($_POST['update_display_position'])) {
    $project_id = $_POST['project_id'];
    $stmt = $pdo->prepare("SELECT display_position FROM project WHERE id = ?");
    ...
    try {
        $stmt = $pdo->prepare("UPDATE project SET display_position = ? WHERE id = ?");
        $stmt->execute([$display_position, $project_id]);
...
if (isset($_POST['remove_display_position'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE project SET display_position = ? WHERE id = ?");
        $stmt->execute([$new_positions, $project_id]);
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        ...
        $_SESSION['toast_message'] = $result ? 'Xóa dự án thành công!' : 'Xóa dự án thất bại!';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['add_project']) || isset($_POST['edit_project']))) {
    ...
    try {
        ...
        if (!empty($_FILES['gallery_images']['name'][0])) {
            ...
            if (isset($_POST['edit_project'])) {
                $project_id = $_POST['project_id'];
                $stmt = $pdo->prepare("SELECT gallery_images FROM project WHERE id = ?");
                $stmt->execute([$project_id]);
        ...
        if (isset($_POST['add_project'])) {
            $stmt = $pdo->prepare("INSERT INTO project (title_vi, parent_id, module_id, display_position, is_active, h1_content, description_vi, content_vi, slug_vi, link_vi, link_target, seo_title_vi, seo_description_vi, seo_keywords_vi, gallery_images, position, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            ...
            log_debug("Added project: $title_vi");
        } else {
            $project_id = $_POST['project_id'];
            $stmt = $pdo->prepare("UPDATE project SET title_vi = ?, parent_id = ?, module_id = ?, display_position = ?, is_active = ?, h1_content = ?, description_vi = ?, content_vi = ?, slug_vi = ?, link_vi = ?, link_target = ?, seo_title_vi = ?, seo_description_vi = ?, seo_keywords_vi = ?, gallery_images = ?, position = ? WHERE id = ?");
            $stmt->execute([$title_vi, $parent_id, $module_id, $display_position, $is_active, $h1_content, $description_vi, $content_vi, $slug_vi, $link_vi, $link_target, $seo_title_vi, $seo_description_vi, $seo_keywords_vi, $gallery_images, $position, $project_id]);
            log_debug("Updated project ID: $project_id");
        }

        $pdo->commit();
        $_SESSION['toast_message'] = isset($_POST['add_project']) ? 'Thêm dự án thành công!' : 'Cập nhật dự án thành công!';
        $_SESSION['toast_type'] = 'success';
    } catch (Exception $e) {
        $pdo->rollBack();
...
// Fetch parent categories (only from module_id = 7 and active)
$stmt = $pdo->query("SELECT id, title_vi FROM categories WHERE module_id = 7 AND is_active = 1 ORDER BY position ASC");
...
$query = "SELECT * FROM project WHERE " . implode(' AND ', $where) . " ORDER BY position ASC, id DESC";
...
<head>
    ...
    <title>Quản lý Dự án - Kaiadmin</title>
                ...
                <div class="page-inner">
                    <div class="page-header">
                        ...
                        <ul class="breadcrumbs mb-3">
                            ...
                            <li class="nav-item"><a href="#">Dự án</a></li>
                        </ul>
                    </div>
                    <div class="row">
                        <?php if ($method === 'frm'): ?>
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h4 class="card-title"><?php echo $edit_project ? 'Sửa Dự án' : 'Thêm Dự án'; ?></h4>
                                    </div>
                                    <div class="card-body">
                                        <form method="POST" enctype="multipart/form-data">
                                            <div class="form-header d-flex justify-content-end">
                                                <button type="submit" name="<?php echo $edit_project ? 'edit_project' : 'add_project'; ?>" class="btn btn-primary">Lưu</button>
                                                <a href="project.php" class="btn btn-secondary ml-2">Hủy</a>
                                            </div>
                                            <div class="row">
                                                <!-- Left Column (3/12) -->
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" value="Dự án (ID: 8)" readonly>
                                                    </div>
                                                    <div class="form-group">
                                                        ...
                                                        <select id="parent_id" name="parent_id" class="form-control">
                                                            ...
                                                            <?php foreach ($parent_categories as $parent): ?>
                                                                <option value="<?php echo $parent['id']; ?>" <?php echo $edit_project && $edit_project['parent_id'] == $parent['id'] ? 'selected' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <?php foreach ($display_positions as $key => $value): ?>
                                                            <div>
                                                                <input type="checkbox" name="display_position[]" id="dp_<?php echo $key; ?>" value="<?php echo $key; ?>" <?php echo $edit_project && in_array($key, explode(',', $edit_project['display_position'] ?? '')) ? 'checked' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div>
                                                            <input type="checkbox" name="is_active" id="is_active" value="1" <?php echo $edit_project && $edit_project['is_active'] ? 'checked' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <?php if ($edit_project && $edit_project['gallery_images']): ?>
                                                            <div class="gallery-preview">
                                                                <?php foreach (explode(',', $edit_project['gallery_images']) as $img): ?>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" id="position" name="position" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['position']) : '0'; ?>" required min="0">
                                                    </div>
                                                </div>
                                                <!-- Right Column (7/12) -->
                                                <div class="col-md-9">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" id="title_vi" name="title_vi" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['title_vi']) : ''; ?>" required placeholder="Nhập tên dự án">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="h1_content">Nội dung thẻ H1</label>
                                                        <input type="text" id="h1_content" name="h1_content" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['h1_content']) : ''; ?>" placeholder="Nhập nội dung H1">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="description_vi">Mô tả tiếng Việt</label>
                                                        <textarea id="description_vi" name="description_vi" class="form-control"><?php echo $edit_project ? htmlspecialchars($edit_project['description_vi']) : ''; ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="content_vi">Nội dung tiếng Việt</label>
                                                        <textarea id="content_vi" name="content_vi" class="form-control"><?php echo $edit_project ? htmlspecialchars($edit_project['content_vi']) : ''; ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="slug_vi">Đường dẫn tiếng Việt</label>
                                                        <input type="text" id="slug_vi" name="slug_vi" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['slug_vi']) : ''; ?>" placeholder="Tự sinh từ tên nếu để trống">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="link_vi">Liên kết tiếng Việt</label>
                                                        <input type="text" id="link_vi" name="link_vi" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['link_vi']) : ''; ?>" placeholder="VD: /du-an">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="link_target">Phương thức</label>
                                                        <select id="link_target" name="link_target" class="form-control">
                                                            <option value="_self" <?php echo $edit_project && $edit_project['link_target'] == '_self' ? 'selected' : ''; ?>>Mở trang hiện tại</option>
                                                            <option value="_blank" <?php echo $edit_project && $edit_project['link_target'] == '_blank' ? 'selected' : ''; ?>>Mở trang mới</option>
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_title_vi">Tiêu đề trang SEO</label>
                                                        <input type="text" id="seo_title_vi" name="seo_title_vi" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['seo_title_vi']) : ''; ?>" placeholder="Nhập tiêu đề SEO">
                                                        <span class="char-count" id="seo_title_count">0/70 ký tự</span>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_description_vi">Mô tả meta SEO</label>
                                                        <textarea id="seo_description_vi" name="seo_description_vi" class="form-control"><?php echo $edit_project ? htmlspecialchars($edit_project['seo_description_vi']) : ''; ?></textarea>
                                                        <span class="char-count" id="seo_description_count">0/160 ký tự</span>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_keywords_vi">Từ khóa meta</label>
                                                        <input type="text" id="seo_keywords_vi" name="seo_keywords_vi" class="form-control" value="<?php echo $edit_project ? htmlspecialchars($edit_project['seo_keywords_vi']) : ''; ?>" placeholder="Nhập từ khóa, cách nhau bằng dấu phẩy">
                                                    </div>
                                                </div>
                                            </div>
                                            <?php if ($edit_project): ?>
                        ...
                        <?php else: ?>
                            <div class="col-md-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <a href="?method=frm" class="btn btn-primary add-btn">Thêm Dự án</a>
                                                ...
                                                <tbody>
                                                    <?php foreach ($projects as $project): ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <form method="POST" action="project.php" class="d-flex align-items-center gap-2">
                                                                    <input type="hidden" name="project_id" value="<?php echo $project['id']; ?>">
                                                                    <input type="hidden" name="update_position" value="1">
                                                                    <input type="number" class="form-control form-control-sm" name="new_position" value="<?php echo htmlspecialchars($project['position']); ?>" min="0" style="width: 80px;">
                                                                    <button type="submit" class="btn btn-sm btn-primary">Cập nhật</button>
                                                                </form>
                                                            </td>

                                                            <td><?php echo htmlspecialchars($project['title_vi']); ?></td>
                                                            <td>
                                                                <?php if ($project['gallery_images']): ?>
                                                                    <div class="gallery-preview">
                                                                        <?php foreach (explode(',', $project['gallery_images']) as $img): ?>
                                                                            <img src="<?php echo htmlspecialchars($img); ?>" alt="Gallery Image">
                                                                        <?php endforeach; ?>
                                                                    </div>
                                                                <?php else: ?>
                                                            ...
                                                            <td>
                                                                <form method="POST" action="project.php">
                                                                    ...
                                                                    <input type="checkbox" name="is_active" class="toggle-switch" value="1" <?php echo $project['is_active'] == 1 ? 'checked' : ''; ?> onchange="this.form.submit()">
                                                                </form>
                                                            </td>
                                                            <td>
                                                                <a href="?method=frm&edit_id=<?php echo $project['id']; ?>" class="btn btn-sm btn-warning">Sửa</a>
                                                                <a href="?delete_id=<?php echo $project['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Bạn có chắc muốn xóa?')">Xóa</a>
                                                            </td>
                                                            <td>
                                                                <div class="display-position-list">
                                                                    <?php if ($project['display_position']): ?>
                                                                        <?php foreach (explode(',', $project['display_position']) as $pos): ?>
