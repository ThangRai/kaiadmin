...
function loadImages() {
    ...
    if (file_exists($useruploadpath)) {
        ...
        for ($i = count($files) - 1; $i >= 0; $i--):
            ...
            $filesizetemp = round($file_size_byte / 1024, 1); // KB
            $totalFileSize += $filesizetemp;
            $count++;
...
<div class="imgDiv"><img class="fileImg" src="<?php echo $image_url; ?>"></div>                    <p class="fileDescription"><span class="fileMime"><?php echo $image_extension; ?></span> <?php echo $image_filename; ?><?php if($file_extens == "yes"){echo ".$image_extension";} ?></p>
        ...
        if ($totalFileSize == 0) {
            ...
            $filesizefinal = round($totalFileSize / 1024, 1) . " MB";
        } else {
            $filesizefinal = round($totalFileSize, 1) . " KB";
        }
