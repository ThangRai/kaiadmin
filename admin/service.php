...
if (isset($_POST['toggle_status'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE service SET is_active = ? WHERE id = ?");
...
if (isset($_POST['update_position'])) {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE service SET position = ? WHERE id = ?");
        $stmt->execute([$new_position, $service_id]);
...
if (isset($_POST['update_display_position'])) {
    $service_id = $_POST['service_id'];
    $stmt = $pdo->prepare("SELECT display_position FROM service WHERE id = ?");
    ...
    try {
        $stmt = $pdo->prepare("UPDATE service SET display_position = ? WHERE id = ?");
        $stmt->execute([$display_position, $service_id]);
...
if (isset($_POST['remove_display_position'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("UPDATE service SET display_position = ? WHERE id = ?");
        $stmt->execute([$new_positions, $service_id]);
...
if (isset($_GET['delete_id'])) {
    ...
    try {
        ...
        $_SESSION['toast_message'] = $result ? 'Xóa dịch vụ thành công!' : 'Xóa dịch vụ thất bại!';
    ...
    try {
        ...
        if (!empty($_FILES['gallery_images']['name'][0])) {
            ...
            if (isset($_POST['edit_service'])) {
                $service_id = $_POST['service_id'];
                $stmt = $pdo->prepare("SELECT gallery_images FROM service WHERE id = ?");
                $stmt->execute([$service_id]);
        ...
        if (isset($_POST['add_service'])) {
            $stmt = $pdo->prepare("INSERT INTO service (title_vi, parent_id, module_id, display_position, is_active, h1_content, description_vi, content_vi, slug_vi, link_vi, link_target, seo_title_vi, seo_description_vi, seo_keywords_vi, gallery_images, position, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            ...
            log_debug("Added service: $title_vi");
        } else {
            $service_id = $_POST['service_id'];
            $stmt = $pdo->prepare("UPDATE service SET title_vi = ?, parent_id = ?, module_id = ?, display_position = ?, is_active = ?, h1_content = ?, description_vi = ?, content_vi = ?, slug_vi = ?, link_vi = ?, link_target = ?, seo_title_vi = ?, seo_description_vi = ?, seo_keywords_vi = ?, gallery_images = ?, position = ? WHERE id = ?");
            $stmt->execute([$title_vi, $parent_id, $module_id, $display_position, $is_active, $h1_content, $description_vi, $content_vi, $slug_vi, $link_vi, $link_target, $seo_title_vi, $seo_description_vi, $seo_keywords_vi, $gallery_images, $position, $service_id]);
            log_debug("Updated service ID: $service_id");
        }

        $pdo->commit();
        $_SESSION['toast_message'] = isset($_POST['add_service']) ? 'Thêm dịch vụ thành công!' : 'Cập nhật dịch vụ thành công!';
        $_SESSION['toast_type'] = 'success';
    } catch (Exception $e) {
        $pdo->rollBack();
...
// Fetch parent categories (only from module_id = 5 and active)
$stmt = $pdo->query("SELECT id, title_vi FROM categories WHERE module_id = 5 AND is_active = 1 ORDER BY position ASC");
...
$query = "SELECT * FROM service WHERE " . implode(' AND ', $where) . " ORDER BY position ASC, id DESC";
...
<head>
    ...
    <title>Quản lý Dịch vụ - Kaiadmin</title>
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header">
                                        <h4 class="card-title"><?php echo $edit_service ? 'Sửa Dịch vụ' : 'Thêm Dịch vụ'; ?></h4>
                                    </div>
                                    <div class="card-body">
                                        <form method="POST" enctype="multipart/form-data">
                                            <div class="form-header d-flex justify-content-end">
                                                <button type="submit" name="<?php echo $edit_service ? 'edit_service' : 'add_service'; ?>" class="btn btn-primary">Lưu</button>
                                                <a href="service.php" class="btn btn-secondary ml-2">Hủy</a>
                                            </div>
                                            <div class="row">
                                                <!-- Left Column (3/12) -->
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" value="Dịch vụ (ID: 5)" readonly>
                                                    </div>
                                                    <div class="form-group">
                                                        ...
                                                        <select id="parent_id" name="parent_id" class="form-control">
                                                            ...
                                                            <?php foreach ($parent_categories as $parent): ?>
                                                                <option value="<?php echo $parent['id']; ?>" <?php echo $edit_service && $edit_service['parent_id'] == $parent['id'] ? 'selected' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <?php foreach ($display_positions as $key => $value): ?>
                                                            <div>
                                                                <input type="checkbox" name="display_position[]" id="dp_<?php echo $key; ?>" value="<?php echo $key; ?>" <?php echo $edit_service && in_array($key, explode(',', $edit_service['display_position'] ?? '')) ? 'checked' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div>
                                                            <input type="checkbox" name="is_active" id="is_active" value="1" <?php echo $edit_service && $edit_service['is_active'] ? 'checked' : ''; ?>>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <?php if ($edit_service && $edit_service['gallery_images']): ?>
                                                            <div class="gallery-preview">
                                                                <?php foreach (explode(',', $edit_service['gallery_images']) as $img): ?>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <input type="number" id="position" name="position" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['position']) : '0'; ?>" required min="0">
                                                    </div>
                                                </div>
                                                <!-- Right Column (7/12) -->
                                                <div class="col-md-9">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" id="title_vi" name="title_vi" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['title_vi']) : ''; ?>" required placeholder="Nhập tên dịch vụ">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="h1_content">Nội dung thẻ H1</label>
                                                        <input type="text" id="h1_content" name="h1_content" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['h1_content']) : ''; ?>" placeholder="Nhập nội dung H1">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="description_vi">Mô tả tiếng Việt</label>
                                                        <textarea id="description_vi" name="description_vi" class="form-control"><?php echo $edit_service ? htmlspecialchars($edit_service['description_vi']) : ''; ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="content_vi">Nội dung tiếng Việt</label>
                                                        <textarea id="content_vi" name="content_vi" class="form-control"><?php echo $edit_service ? htmlspecialchars($edit_service['content_vi']) : ''; ?></textarea>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="slug_vi">Đường dẫn tiếng Việt</label>
                                                        <input type="text" id="slug_vi" name="slug_vi" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['slug_vi']) : ''; ?>" placeholder="Tự sinh từ tên nếu để trống">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="link_vi">Liên kết tiếng Việt</label>
                                                        <input type="text" id="link_vi" name="link_vi" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['link_vi']) : ''; ?>" placeholder="VD: /dich-vu">
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="link_target">Phương thức</label>
                                                        <select id="link_target" name="link_target" class="form-control">
                                                            <option value="_self" <?php echo $edit_service && $edit_service['link_target'] == '_self' ? 'selected' : ''; ?>>Mở trang hiện tại</option>
                                                            <option value="_blank" <?php echo $edit_service && $edit_service['link_target'] == '_blank' ? 'selected' : ''; ?>>Mở trang mới</option>
                                                        </select>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_title_vi">Tiêu đề trang SEO</label>
                                                        <input type="text" id="seo_title_vi" name="seo_title_vi" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['seo_title_vi']) : ''; ?>" placeholder="Nhập tiêu đề SEO">
                                                        <span class="char-count" id="seo_title_count">0/70 ký tự</span>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_description_vi">Mô tả meta SEO</label>
                                                        <textarea id="seo_description_vi" name="seo_description_vi" class="form-control"><?php echo $edit_service ? htmlspecialchars($edit_service['seo_description_vi']) : ''; ?></textarea>
                                                        <span class="char-count" id="seo_description_count">0/160 ký tự</span>
                                                    </div>
                                                    <div class="form-group">
                                                        <label for="seo_keywords_vi">Từ khóa meta</label>
                                                        <input type="text" id="seo_keywords_vi" name="seo_keywords_vi" class="form-control" value="<?php echo $edit_service ? htmlspecialchars($edit_service['seo_keywords_vi']) : ''; ?>" placeholder="Nhập từ khóa, cách nhau bằng dấu phẩy">
                                                    </div>
                                                </div>
                                            </div>
                                            <?php if ($edit_service): ?>
                        ...
                        <?php else: ?>
                            <div class="col-md-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <a href="?method=frm" class="btn btn-primary add-btn">Thêm Dịch vụ</a>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-2">
                                                    <div class="form-group mt-4">
                                                        ...
                                                        <a href="service.php" class="btn btn-secondary">Reset</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </form>
                                                ...
                                                <tbody>
                                                    <?php foreach ($services as $service): ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <form method="POST" action="service.php" class="d-flex align-items-center gap-2">
                                                                    <input type="hidden" name="service_id" value="<?php echo $service['id']; ?>">
                                                                    <input type="hidden" name="update_position" value="1">
                                                                    <input type="number" class="form-control form-control-sm" name="new_position" value="<?php echo htmlspecialchars($service['position']); ?>" min="0" style="width: 80px;">
                                                                    <button type="submit" class="btn btn-sm btn-primary">Cập nhật</button>
                                                                </form>
                                                            </td>

                                                            <td><?php echo htmlspecialchars($service['title_vi']); ?></td>
                                                            <td>
                                                                <?php if ($service['gallery_images']): ?>
                                                                    <div class="gallery-preview">
                                                                        <?php foreach (explode(',', $service['gallery_images']) as $img): ?>
                                                                            <img src="<?php echo htmlspecialchars($img); ?>" alt="Gallery Image">
                                                                        <?php endforeach; ?>
                                                                    </div>
                                                                <?php else: ?>
                                                            ...
                                                            <td>
                                                                <form method="POST" action="service.php">
                                                                    ...
                                                                    <input type="checkbox" name="is_active" class="toggle-switch" value="1" <?php echo $service['is_active'] == 1 ? 'checked' : ''; ?> onchange="this.form.submit()">
                                                                </form>
                                                            </td>
                                                            <td>
                                                                <a href="?method=frm&edit_id=<?php echo $service['id']; ?>" class="btn btn-sm btn-warning">Sửa</a>
                                                                <a href="?delete_id=<?php echo $service['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Bạn có chắc muốn xóa?')">Xóa</a>
                                                            </td>
                                                            <td>
                                                                <div class="display-position-list">
                                                                    <?php if ($service['display_position']): ?>
                                                                        <?php foreach (explode(',', $service['display_position']) as $pos): ?>
